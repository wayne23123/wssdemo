...
<template>
  <div class="pageOneLayOut">
    ...
    <div v-if="adminCartStore.orderSearchTermRef.length < 1" class="dataButton">
      <div @click="adminCartStore.reverseOrderRefs" class="dataButtonOrder">
        訂單編號▼
      </div>
    ...
    <div class="dataLayout">
      ...
      >
        ...
        <div v-if="adminCartStore.orderSearchTermRef.length < 1">
      ...
      >
        <div class="cardTitle">
          ...
          <button @mouseenter="completeTrue(searchs[0].order)">
            完成訂單 ✓
          </button>
          <button @mouseenter="completeFalse(searchs[0].order)">
            未完成訂單 ✕
          </button>
        </div>
        <table>
